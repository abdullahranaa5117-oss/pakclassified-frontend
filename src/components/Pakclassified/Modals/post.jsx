...
function PostAdForm({ show, handleClose }) {
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_BASE_URL}/api/v1/Advertisement`, {
        method: "POST",
        headers: {
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control type="file" accept="images/*" {...register("Images", { required: "Images is required" })} />
            {errors.Images && <Form.Text className="text-danger">{errors.Images.message}</Form.Text>}
          </Form.Group>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={handleClose}>Close</Button>
          <Button variant="success" type="submit" disabled={isSubmitting}>Post Advertisement</Button>
        </Modal.Footer>
      </Form>
    </Modal>
  );
